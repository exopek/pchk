...
//#include "../helper/include/errors.h"
	...
	: m_status(CLOSED)
{
	...
	m_comMode = RS232_MODE_STD | RS232_MODE_BIT_PCK_CD_CTS | RS232_MODE_BIT_PCK_CHECK_DSR;//RS232_MODE_STD | RS232_MODE_BIT_PCK_CD_CTS;
	m_comRTSMode = RS232_RTSMODE_TOGGLE;//RS232_RTSMODE_ALWAYSON;
...
void Rs232Device::Open()
{
	if (GetStatus() != Rs232Device::OPENED)
	{
		...
		// --------------------Ausgabe port und baudrate--------------------------
		//printf(port);
		//std::string tmp = std::to_string(static_cast<long long>(baudrate));
...
int Rs232Device::Read(char* buf, unsigned size) const
{
	...
	if (comStat.cbInQue > 0)
	{
		...
		DWORD syncHaveRead = 0; // Number of bytes read
...
bool Rs232Device::CreateHandle(const char* device)
{
	...
	// API windows für seriel
	//printf("Device Name: %s", &deviceName);
	HandleValue h = CreateFileA(deviceName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
	//HandleValue h = ::CreateFileA(deviceName, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_FLAG_OVERLAPPED, NULL);
	...
	if (h == INVALID_HANDLE_VALUE) // nicht-fataler Fehler (-> USB!)
	{
		...
		//throw RS232Exception(ERR_IO_Open, deviceName, ::GetLastError());
...
{
	// Settings für den seriellen Port 
	DCB dcb;
